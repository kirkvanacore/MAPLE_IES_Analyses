...
* Daagonbox is sig lower than all conditions 
  * not this is not true when taking into account the bonf corrected p value (.0125)
...
* distributions are bimodal suggesting two groups: high and low fertility
* Group Differences:
...
## Do schools/class differ in terms of implemntation?
...
### Variance in fertility by school
ggplot(
...
### Variance in fertility by school and condition
ggplot(
  assess[is.na(assess$final_school_id) == F, ],
  aes(x = final_school_id, 
      y = fidelity_complete_sum, 
...
### Variance in fertility by school and implementation model (remote/in-person/) 
...
Relavant Variables
* MEMBERSHIPDAYS6	AbsentDays + PresentDays
* A DM6 ->	MembershipDays / 178
* ADA6 ->	PresentDays / 178
* Absencesavg.Daily6 ->	UnexcusedDays / 179
* PercentInAttendance6 ->	PresentDays / MembershipDays
* Absencesavg.Daily5
...
```{r}
describe(assess$PercentInAttendance5)
describe(assess$PercentInAttendance6)


# denisity plot by condition
ggplot(
  data = assess,
  aes(x = PercentInAttendance6,
      group = condition_assignment,
      fill = condition_assignment)
) +
  geom_density(adjust = 1.5, alpha = .4)
...
## Questions:
 * Is there a variable for the students grade during the study?
 * Are their definitions for the fidelity variables (fidelity_complete_percent, fidelity_complete_sum, fidelity_started_sum)? 
    * Do they have to do with the proportion/number of assignments that a student completed/started?
    * If so, why is the max number of fidelity_complete_sum 13, but the complete_assignment variables only go up to 11?
    * Where can I get more information about the assignments and how they worked?
    
    
    
     fedility as: did students go through the motions vs fedility as did students engage in th learning proccess
